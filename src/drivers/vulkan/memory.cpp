...
namespace hydra {
    namespace drivers {
        namespace vlkn {
            ...
            AllocatedBuffer MemoryAllocator::allocateBuffer(vk::BufferCreateInfo bufferCreateInfo, vk::MemoryPropertyFlags memflags)
            {
                vk::MemoryAllocateInfo info{};
                ...
                // imageCreateInfo.
                vk::DeviceBufferMemoryRequirements devMemReq{};
                devMemReq.pCreateInfo = &bufferCreateInfo;
                ...
                // std::cout << (int)imageCreateInfo.format << std::endl;
                // std::cout << (int)imageCreateInfo.imageType << std::endl;
                auto memreq = m_basicDataPtr->device->getBufferMemoryRequirements(devMemReq).memoryRequirements;
                auto memprops = m_basicDataPtr->physicalDevice->getMemoryProperties();
                auto memoryTypeBits = memreq.memoryTypeBits;
                info.memoryTypeIndex = -1;
                for (int i = 0; i != memprops.memoryTypeCount; ++i, memoryTypeBits >>= 1) {
                ...
                info.allocationSize = memreq.size;
                auto devMem = vk::raii::DeviceMemory(*m_basicDataPtr->device, info);
                auto buffer = vk::raii::Buffer(*m_basicDataPtr->device, bufferCreateInfo);

                m_basicDataPtr->device->bindBufferMemory2({vk::BindBufferMemoryInfo(*buffer, *devMem, 0)});

                return {std::move(buffer), std::move(devMem), info.allocationSize};
            }
        }  // namespace vlkn
    }      // namespace drivers
}  // namespace hydra
